...
def binary_search(searching_list: list[int], number: int):
    ...
    while searching_list[current_point] != number and runtime:
        if number < searching_list[current_point]:
            last_highpoint = current_point
            current_point = int((current_point + last_low_point)/2)
        else:
            last_low_point = current_point
            current_point = int((current_point + last_highpoint)/2)
        runtime -= 1
    if number == searching_list[current_point]:
